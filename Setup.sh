...
# =================================================================
#  Linux Pentesting Environment Setup Script
# =================================================================
# Description: Automated setup for Linux and other security
...
create_directories() {
    ...
    if [[ -f "$(dirname "$0")/directories.sh" ]]; then
        source "$(dirname "$0")/directories.sh" 
    else
        print_warning "directories.sh not found, skipping..." 
        # Fallback for directory creation if directories.sh is missing.
        # This is not from sources, but ensures basic pentesting directory exists.
        mkdir -p "$HOME/pentesting" "$HOME/pentesting/tools" "$HOME/pentesting/scripts" "$HOME/pentesting/wordlists" "$HOME/pentesting/targets" "$HOME/pentesting/reports" "$HOME/pentesting/data"
        mkdir -p "$HOME/venvs"
    fi
    # Create all directories (assuming create_directory_structure is sourced or provided) 
    create_directory_structure || print_warning "create_directory_structure function not found or failed."
...
update_system() {
    ...
    sudo "$PKG_MANAGER" update && sudo "$PKG_MANAGER" upgrade -y  # Use PKG_MANAGER variable
...
declare -g -A TOOL_CONFIGS # This variable is used by install_github_tools , assuming it's populated elsewhere for the full script. 
...
install_pkg() {
    ...
    while [ "$attempt" -le "$retries" ]; do
        ...
        case "$PKG_MANAGER" in
            apt)
                if sudo DEBIAN_FRONTEND=noninteractive apt install -y "$pkg" >/dev/null 2>>"$LOG_FILE"; then 
...
verify_artifact() {
    ...
    case "$type" in
        gpg)
            verify_gpg_signature "$file" "$file.asc" "$reference" || { print_error "$file.asc not found or GPG verification failed"; return 1; }  # Corrected GPG call
            ;;
        sha256)
...
install_tool() {
    ...
    local usage="Usage: install_tool [options] | install_tool --config=<yaml_config_path>
Direct installation:
  install_tool --source=<type> --url=<repo_url_or_package_name> [options]
YAML configuration:
  install_tool --config=<yaml_config_path>
Options:
  ...
  --install   Install command" 
    ...
    if [[ "$1" == "--config="* ]]; then
        config="${1#--config=}" 
        # parse_yaml_config and actual YAML definition not provided in sources.
        print_error "YAML configuration parsing function (parse_yaml_config) not provided in sources. Cannot install via config."
        return 1
    else
        name="$1" 
        shift
    ...
    # install_from_git and install_from_package functions are not provided in sources.
    case "${tool_config[source]}" in
        github|gitlab)
            print_error "install_from_git function not provided in sources. Skipping git install for $name."
            return 1
            ;;
        pypi|go)
            print_error "install_from_package function not provided in sources. Skipping package install for $name."
            return 1
            ;;
        custom)
...
install_github_tools() {
    ...
    local -a pids=()
    # TOOL_CONFIGS is declared but its content is not provided, making this function inoperable as-is from sources.
    print_warning "Skipping GitHub tools installation via TOOL_CONFIGS due to missing YAML parsing or tool definitions in the provided sources."
...
validate_packages() {
    ...
    # The `PACKAGES` array is not defined in the provided source excerpts.
    print_warning "Skipping package validation as PACKAGES array is not defined in provided sources."
    # local missing=()
    # case "$PKG_MANAGER" in
    #     apt)
...
# Inicializar todas las variables que podrían no estar definidas
SELECTED_MODULES="${SELECTED_MODULES:-}"
OPTIONS="${OPTIONS:-}"

# Inicializar flags de skip
SKIP_TOOLS=true
SKIP_PYTHON=true
SKIP_ZSH=true
SKIP_DOCKER=true
...
# Actualizar flags según módulos seleccionados
...
# Configuración de entorno virtual automático
AUTO_ACTIVATE_VENV=false
[[ $OPTIONS == *AUTO_VENV* ]] && AUTO_ACTIVATE_VENV=true
...
qscan() { if [[ -z "$1" ]]; then echo "Usage: qscan <target_ip>"; return 1; fi; nmap -sS -sV -O -T4 --top-ports 1000 "$1"; }
...
setup_auto_venv() {
    ...
    local zshrc_path="${XDG_CONFIG_HOME:-$HOME/.config}/zsh/.zshrc"
    if ! grep -q "source ~/venvs/pentesting/bin/activate" "$zshrc_path" 2>/dev/null; then # 
        echo "" >> "$zshrc_path"
        echo "# Auto-activate pentesting Python venv" >> "$zshrc_path"
        echo "[[ -f ~/venvs/pentesting/bin/activate ]] && source ~/venvs/pentesting/bin/activate" >> "$zshrc_path" 
...
create_useful_scripts() {
    ...
    mkdir -p "$HOME/pentesting/scripts" # Ensure scripts directory exists
    # Port scanner script 
    cat > "$HOME/pentesting/scripts/portscan.sh" << 'EOF'
#!/bin/bash
...
EOF
    ...
    cat > "$HOME/pentesting/scripts/webenum.sh" << 'EOF'
#!/bin/bash
...
EOF
    ...
    cat > "$HOME/pentesting/scripts/smbenum.sh" << 'EOF'
#!/bin/bash
# SMB enumeration script
...
EOF
    ...
    chmod +x "$HOME/pentesting/scripts"/*.sh 
...
setup_wordlists() {
    ...
    mkdir -p "$HOME/pentesting/wordlists" # Ensure wordlists directory exists
    cd "$HOME/pentesting/wordlists" || { print_error "Failed to change to wordlists directory"; return 1; }
...
install_advanced_pentest_tools() {
    ...
    rm kubectl 
    mkdir -p "$HOME/pentesting/tools"
    cd "$HOME/pentesting/tools" || { print_error "Failed to change to tools directory"; return 1; }
...
create_advanced_scripts() {
    ...
    cat > "$HOME/pentesting/scripts/init_target.sh" << 'EOF'
#!/bin/bash
if [ $# -eq 0 ]; then
echo "Usage: $0 [IP]"
exit 1
...
EOF
    ...
    cat > "$HOME/pentesting/scripts/discover_network.sh" << 'EOF'
#!/bin/bash
if [ $# -eq 0 ]; then
...
EOF
    ...
    cat > "$HOME/pentesting/scripts/privesc_check.sh" << 'EOF'
#!/bin/bash
echo "=========================================="
...
# Global variables for interactive selection. Initialized to skip everything by default.
SKIP_TOOLS=true
SKIP_PYTHON=true
SKIP_ZSH=true
SKIP_DOCKER=true
...
EOF
    ...
    # **This is the key fix: Call the interactive menu to let the user select modules**
    select_modules_interactive 

    print_status "Starting modular installation based on your selections..."

    # System update (recommended before installing packages) 
    update_system 

    # Conditional installation based on user choices from the interactive menu
    if [[ "$SKIP_TOOLS" == "false" ]]; then
        print_status "Installing core tools and packages..."
        install_essential_packages 
        install_github_tools  # Will be skipped with warning due to missing internal functions.
        configure_metasploit 
        create_useful_scripts 
        configure_proxychains 
    fi
    ...
    if [[ "$SKIP_ZSH" == "false" ]]; then
        print_status "Setting up Zsh environment and modern CLI tools..."
        install_modern_cli_tools 
        setup_unified_aliases 
        setup_advanced_aliases 
        setup_tmux_config 
        setup_zsh_environment  # This function sets up XDG, OMZ, and moves the alias files created above.
        if [[ "$AUTO_ACTIVATE_VENV" == "true" ]]; then
            setup_auto_venv # This appends to the newly created XDG .zshrc
        fi
    fi

    if [[ "$SKIP_PYTHON" == "false" ]]; then
        # Python prerequisites (python3-pip, python3-venv, python3-dev) are handled by install_essential_packages.
        # The actual creation of 'pentesting' and 'webapp' virtual environments is not defined in provided sources.
        print_status "Python environments selected."
        print_warning "Actual creation of specific Python virtual environments (e.g., 'pentesting', 'webapp') needs manual steps or a 'python.sh' script not provided in sources."
    fi
    ...
    if [[ "$SKIP_ADVANCED" == "false" ]]; then
        print_status "Installing advanced pentesting tools and scripts..."
        install_advanced_pentest_tools 
        create_advanced_scripts 
    fi
